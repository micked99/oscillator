...
void setup() {
  ...
  pinMode(5, OUTPUT);
  pinMode(6, OUTPUT);
  pinMode(7, OUTPUT);// Set PD4 pin high = Arduino pin4
  digitalWrite(4, HIGH);
  digitalWrite(5, HIGH);
  digitalWrite(6, HIGH);
  digitalWrite(7, HIGH);// Sets the digital pin 4 on = Enable
  delay(100);
  ...
  digitalWrite(SCL, 1);                                    
  SetFrequency (14097000);              // Set TX-Frequency [14,095 MHz]
...
void SetFrequency (unsigned long frequency) { // Frequency in Hz; must be within [7810 Hz to 200 Mhz]
  #define F_XTAL 26000000;             // Frequency of Quartz-Oszillator
...
void SetPower (byte power){                         // Sets the output power level
  if (power == 1 || power > 4){power = 0;}          // valid power values are 0 (25%), 1 (50%), 2 (75%) or 3 (100%)
  switch (power){
    case 1:
